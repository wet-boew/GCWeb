     ...
     */
	let wait;
    var componentName = "gc-collection-sort",
    	...
    	init = function( event ) {
    		...
    		if ( elm ) {
    			...
    			// Get the plugin JSON configuration set on attribute data-gc-collection-sort
    			settings = $.extend(
    				true,
    				{},
    				defaults,
    			...
    			$elm.trigger( "gc-collection-sort", settings );
    			// Identify that initialization has completed
    			wb.ready( $elm, componentName );
    		}
    	};
    // Add your plugin event handler
    $document.on( "gc-collection-sort", selector, function( event, data ) {
			...
			sortContainers.forEach(function(container){
				...
				data.sort.forEach( function(sort) {			
					...
					sortItems.forEach( function (element) {
						...
						let sortObj = { 
							"elm" : element,
							"sortVal" : element.querySelector(sort.selector).innerHTML
						};
					...
					sortArray.forEach(function(element, index) {
						//sortContainer.append(element.elm);
