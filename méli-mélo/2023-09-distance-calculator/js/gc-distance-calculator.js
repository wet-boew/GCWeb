	 ...
	 */
	var componentName = "gc-distance-calculator",
		...
		init = function( event ) {
			...
			if ( elm ) {
				...
				// Get the plugin JSON configuration set on attribute data-gc-distance-calculator
				settings = $.extend(
					true,
					{},
					defaults,
				...
				$elm.trigger( "gc-distance-calculator", settings );
				// Identify that initialization has completed
				wb.ready( $elm, componentName );
			}
		};
	// Add your plugin event handler
	$document.on( "gc-distance-calculator", selector, function( event, data ) {
		...
		$.getJSON(distAPI + addressEnc, function(json) {
			...
			distCollectionItems.forEach( function(element) {
				...
				if(typeof dist !== "undefined" && dist !== null && typeof distSort !== "undefined" && distSort !== null && typeof dist.dataset.gcDistanceCoordinates !== "undefined" && dist.dataset.gcDistanceCoordinates !== null){
				
					let coordinates = JSON.parse(dist.dataset.gcDistanceCoordinates);
